  ...
  return (
    <div>
      ...
      <p className="text-neutral-400 mb-8">
        All endpoints (except <code className="text-amber-400">/health</code>) require authentication
...
{`{
  ...
  "timeout": 300
      ...
      <Endpoint method="GET" path="/execution/{id}/logs" auth={true}>
        ...
        <p className="mt-2 text-xs text-neutral-500">Content-Type: text/event-stream</p>
      </Endpoint>

      <Endpoint method="GET" path="/executions" auth={true}>
        List recent executions. Supports query parameters:
        <ul className="mt-2 list-disc list-inside text-xs">
          <li><code className="text-amber-400">?limit=N</code> - Number of results (default: 50)</li>
          <li><code className="text-amber-400">?status=RUNNING</code> - Filter by status</li>
        </ul>
      </Endpoint>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-neutral-800">
              <th className="px-4 py-3 text-left text-xs font-medium text-neutral-500 uppercase">Field</th>
      ...
      <h3 className="text-md font-medium text-neutral-200 mb-3">GitHub Actions</h3>
...
{`- name: Deploy to Production
  run: |
    curl -sf -X POST http://\${{ vars.AGENT_HOST }}:8090/execute \\
      ...
      <h3 className="text-md font-medium text-neutral-200 mb-3 mt-6">GitLab CI</h3>
      <CodeBlock language="yaml">
{`deploy:
  stage: deploy
  script:
